...
const Admin = () => {
  ...
  // Find shopkeeper name by ID
  const getShopkeeperName = (shopkeeperId: string) => {
              ...
              {pendingRegistrations.length > 0 ? (
                <div className="space-y-4">
                  {pendingRegistrations.map(registration => (
                    <div key={registration.id} className="p-4 border rounded-md">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <div className="font-medium">{registration.shopName}</div>
                      ...
                      <div className="text-sm mb-3">
                        ...
                        <div><strong>Location:</strong> {registration.location.lat.toFixed(4)}, {registration.location.lng.toFixed(4)}</div>
                        {registration.machineId && (
                          <div><strong>Machine ID:</strong> {registration.machineId}</div>
                        )}
                        <div><strong>Submitted:</strong> {new Date(registration.timestamp).toLocaleString()}</div>
                      </div>
              ...
              <div className="space-y-4">
                <div className="p-4 bg-gray-50 rounded-md">
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    {users.filter(u => u.role === 'customer').length} customers, {users.filter(u => u.role === 'shopkeeper').length} shopkeepers
                  </div>
                </div>

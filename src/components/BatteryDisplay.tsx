...
import { calculateChargingTime } from '@/lib/calculations';
...
const BatteryDisplay = ({
  ...
  const hoursToFullCharge = isCharging ? calculateChargingTime(batteryPercentage, solarEfficiency) : 0;
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-4">
        ...
        <Progress value={batteryPercentage} className="h-3" indicatorClassName={getColor()} />
      </div>
      ...
      <div className="border-t pt-2">
        ...
        <div className="flex items-center justify-around gap-2">
          <div className={`text-center p-2 rounded ${isPaymentMachineOn ? 'bg-sugarcane-light/20 text-sugarcane-dark' : 'bg-gray-100 text-gray-400'}`}>
            <PaymentMachine className="h-5 w-5 mx-auto" />
            <span className="text-xs block mt-1">Payment</span>
          </div>
          
          <div className={`text-center p-2 rounded ${isLightOn ? 'bg-solar-light/20 text-solar-dark' : 'bg-gray-100 text-gray-400'}`}>
            <Lightbulb className="h-5 w-5 mx-auto" />
            <span className="text-xs block mt-1">Light</span>
          </div>
          
          <div className={`text-center p-2 rounded ${fanSpeed !== 'off' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-400'}`}>
            <Fan className={`h-5 w-5 mx-auto ${fanSpeed !== 'off' ? 'animate-spin-slow' : ''}`} />
            <span className="text-xs block mt-1">
              {fanSpeed === 'off' ? 'Fan Off' : `${fanSpeed.charAt(0).toUpperCase() + fanSpeed.slice(1)}`}
            </span>
          </div>
        </div>
      </div>

...
import { calculateSugarcanesNeeded, hasEnoughBattery } from '@/lib/calculations';
import { useToast } from '@/components/ui/use-toast';

const OrderForm = () => {
  const { currentUser, selectedShop, createOrder, getMachineByShopId } = useAppContext();
  const { toast } = useToast();
  const [glassCount, setGlassCount] = useState(1);
  const [isOrdering, setIsOrdering] = useState(false);
  
  const machine = selectedShop ? getMachineByShopId(selectedShop.id) : undefined;
  
  // Calculate required sugarcane
  ...
  const handleOrder = () => {
    ...
    if (!hasEnoughBatteryForOrder) {
      toast({
        ...
        description: "The shop's machine doesn't have enough battery for this order",
        variant: "destructive",
      });
      return;
    }
    ...
    <Card className="shadow-md">
      ...
      <CardContent>
        {selectedShop ? (
          <>
            <div className="mb-4">
              <p className="text-sm font-medium">Selected Shop: {selectedShop.name}</p>
            </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button
                    variant="outline" 
                    size="sm"
                    onClick={() => setGlassCount(prev => Math.max(1, prev - 1))}
                  ...
                  <Input 
                    ...
                    min="1"
                    value={glassCount}
                    onChange={(e) => setGlassCount(parseInt(e.target.value) || 1)}
                  />
                  <Button
                    variant="outline" 
                    size="sm"
              ...
              <div className="bg-gray-50 p-3 rounded-md">
                ...
                {!hasEnoughBatteryForOrder && (
                  <div className="text-xs text-red-500 mt-1">
                    Warning: Low battery. The machine may not be able to complete this order.
                  </div>
                )}
              </div>
